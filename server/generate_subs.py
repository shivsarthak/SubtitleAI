...
app.config['OUTPUT_FOLDER'] = os.environ.get(
    'OUTPUT_FOLDER', os.path.abspath('./tmp/output/'))
app.config['CELERY_BROKER_URL'] = os.environ.get(
    'CELERY_BROKER_URL', 'redis://192.168.0.191:6379')
app.config['CELERY_BACKEND_URL'] = os.environ.get(
    'CELERY_BACKEND_URL', 'redis://192.168.0.191:6379')
...
def generate_subtitles_task(self, video_name, model_size):
    ...
    try:
        ...
        srt_writer = WriteSRT('.')
        with open(srt_file_path, 'w') as out:
            srt_writer.write_result(transcript, out)
        end = datetime.now()
        timetaken = (end-start).seconds

        return f"Executed in: {timetaken} s"
